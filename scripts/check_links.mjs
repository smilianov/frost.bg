...
//   - Markdown: [текст](цел), ![alt](цел) — ВКЛЮЧИТЕЛНО вътре в `inline
//     code` и вътре в <!-- HTML коментари -->: тих пропуск е единственият
//     резултат, който този инструмент не си позволява (виж бележката за
//     редакцията по-долу), а шумен фалшив резултат е поносим — вижда се и
//     се решава за минута;
//   - HTML href/src (само с кавички) — като атрибут на реална позиция
//     (предхожда го празно пространство), не като подниз навсякъде в
//     текста — `<div title='href="/x"'>` не съвпада, както в
//     site/**/*.html, така и вградено в Markdown файл (напр. `<a
//     href="…">` в проза, дори вътре в `inline code`);
...
//     отварящия таг.
//
// Преди разпознаването се редактират (заменят се със същия брой редове, но
// празно съдържание) САМО ```-оградените блокове — примерен код в
// плановете. Умишлено НЕ се редактират `inline code` и HTML коментари: и
// двата опита (кръг 2 на този скрипт) създадоха фалшиво чувство за
// сигурност там, където две отделни неща се пресичат — `` `` `` (единична
// обратна кавичка) вътре в HTML коментар се сдвоява с обратната кавичка на
// съвсем друг `inline code` span по-надолу в реда и изяжда истинска
// Markdown връзка между тях:
//
//   Текст <!-- ` --> [счупена](/missing.html) `код`
//
// Преди тази поправка горното ставаше "0 мъртви връзки" — истинската
// счупена връзка изчезваше тихо. Сега примери, писани с `inline code` или
// <!-- коментари -->, ще излязат като проверени (шумно, ако целта им не
// съществува) — единственият сигурен начин да се пише пример, който
// скриптът наистина не докосва, е ```-ограда.
//
...
// --- редакция: маха само ```-блокове ------------------------------------

// ```-оградени блокове — по цели редове, само в Markdown. Единственото
// нещо, което се редактира тук нарочно (виж бележката най-горе за защо
// `inline code` и HTML коментарите НЕ се редактират повече) — фенсовете са
// котвени за реда, без чужда пунктуация вътре, която да обърка границите.
...
const MD_INLINE = /\]\(([^)]+)\)/g;
// href="…"/src="…" или href='…'/src='…' (само с кавички) — като атрибут на
// реална позиция: изисква да е предшествано от празно пространство (както
// винаги е в HTML — след името на тага или след предния атрибут), не само
// "не буква/тире". Само `[\w-]` пред него не стигаше — `<div
// title='href="/x"'>` е предшествано от кавичка, не от буква, но пак не е
// истински атрибут; сега изисква буквално `\s` пред "href"/"src". Флагът
// "d" дава началото на самата уловена група, за да сочи докладваният ред
// точно към стойността, не към "href"/"src" (може да е на предния ред при
// многоредов таг).
const HTML_ATTR = /(?<=\s)(?:href|src)\s*=\s*"([^"]*)"|(?<=\s)(?:href|src)\s*=\s*'([^']*)'/gd;
