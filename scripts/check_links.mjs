...
//     минава през редове, но без `]` вътре) и адрес или в `<…>`, или без
//     празно пространство и без скоби; незадължителното заглавие след адреса
//     ("…", '…', (…)) не влиза в целта. ВКЛЮЧИТЕЛНО вътре в `inline code` и
//     вътре в <!-- HTML коментари -->;
...
//   - изображение като връзка ([![alt](img)](цел)), етикет с вложени
//     квадратни скоби и адрес със скоби вътре НЕ се проверяват — етикетът не
//     може да съдържа `]`, адресът не може да съдържа скоби. Излизат като
//     „Неразпознато“, тоест шумно: старият хлабав израз ги хващаше случайно,
//     но с цената на изядени истински връзки през края на коментар;
...
//     номерата).
// Няма ли затваряща до края на файла — НИЩО не се зачерква и редът на
// отварящата излиза като проблем: съдържанието се проверява шумно, вместо
// остатъкът от файла да изчезне тихо.
//
...
function redactFences(text) {
  ...
  if (fence) return { text, unclosedFence: fence.line };
...
//   - иска истински `[етикет]` (може да минава през редове, но без `]` вътре)
//     — само `](` хващаше и `](` в HTML коментар и оттам поглъщаше до
//     следващата затваряща скоба, изяждайки истинската връзка между двете;
