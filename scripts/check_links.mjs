...
//   - целта се декодира като URI компонент СЕГМЕНТ ПО СЕГМЕНТ
//     (`docs/real%20file.md` → `docs/real file.md`), преди да опре до
//     файловата система: цял адрес наведнъж значи, че един счупен сегмент
//     (`missing%zz.html`) отменя декодирането и на съседите си, а с него и
//     нормализирането на `%2e%2e`;
...
//   - `%2F` в сегмент НЕ е разделител: декодира се, но сегментът си остава
//     един, иначе `..` изяждат погрешните сегменти (`/api/a%2Fb/../../x`);
//   - адрес, чийто СУРОВ И НОРМАЛИЗИРАН вид И ДВАТА започват с "/api/" →
//     маршрут на Worker-а, не файл — прескача се нарочно (проверка във
//     файловата система би излъгала). Съгласието на двата пази и двете
//     посоки на грешката: `/api/a%2Fb/../../x` не е маршрут, макар суровият
//     му вид да започва с "/api/", а `/api%2Fv1/frost` не е маршрут, макар
//     декодиран да изглежда така — Worker-ът гледа `URL.pathname`, който
//     запазва `%2F`. При разминаване не прескачаме, а проверяваме.
//     Преди нормализирането `/api/../missing.html` се пропускаше
...
// Декодира пътя СЕГМЕНТ ПО СЕГМЕНТ и връща сегментите. Две неща се решават
// тук и двете бяха платени с дефект:
//   - цял адрес наведнъж не става: един счупен сегмент (`missing%zz.html`)
//     караше decodeURIComponent да хвърли и връщаше целия СУРОВ адрес, затова
//     `%2e%2e` в съседния сегмент оставаше ненормализирано;
//   - резултатът НЕ се слепва и не се разделя наново: `a%2Fb` се декодира до
//     `a/b` и второто разделяне го брои за ДВА сегмента, което кара `..` да
//     изяде погрешните. В адрес `%2F` не е разделител — сегментът си остава
//     един, каквото и да има в него.
function decodeSegments(t) {
  return t.split("/").map(safeDecode);
}

// Нормализира "." и ".." в сайт-абсолютен адрес над ДЕКОДИРАНИТЕ сегменти,
// като пази началната и крайната наклонена черта (крайната носи изискването
// „трябва да е директория“). Връща null, ако ".." излиза над началото.
function normalizeAddress(t) {
  const trailing = t.endsWith("/") && t !== "/";
  const out = [];
  for (const seg of decodeSegments(t)) {
    if (seg === "" || seg === ".") continue;
    if (seg === "..") {
...
function resolveTarget(target, sourceFile, siteDir, root) {
  ...
  if (t.startsWith("/")) {
    ...
    // `/api/` се пропуска САМО когато суровият (преди декодиране) и
    // нормализираният път са съгласни. Всяко разминаване значи, че `%2F` или
    // dot segment местят границата на маршрута — тогава не прескачаме, а
    // проверяваме: при неяснота шумно, не тихо. Пази и двете посоки на
    // грешката: `/api/a%2Fb/../../x` не е маршрут, а `/api%2Fv1/frost` не е
    // маршрут, макар декодиран да изглежда така.
    if (t.startsWith("/api/") && norm.startsWith("/api/")) return { skip: "api route" };
    return { path: join(siteDir, norm) };
  }
  // join сам нормализира "." и ".."; сегментите се декодират поотделно, за да
  // не отмени един счупен сегмент декодирането на съседите си.
  const abs = join(dirname(sourceFile), ...decodeSegments(t));
