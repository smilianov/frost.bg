...
//
// Диагностиките идват от B — той е по-точният; при разминаване между двата
// скриптът не мълчи, а съобщава. При файл без коментари двата съвпадат и
// нищо не се променя.
...
// Декодира пътя СЕГМЕНТ ПО СЕГМЕНТ. Цял адрес наведнъж не става: един счупен
// сегмент (`missing%zz.html`) караше decodeURIComponent да хвърли и връщаше
// целия СУРОВ адрес, затова `%2e%2e` в съседния сегмент оставаше
// ненормализирано и `/api/%2e%2e/missing%zz.html` пак минаваше за маршрут на
// Worker-а. Сега счупеният сегмент остава суров, а съседите му се декодират.
function decodePath(t) {
  return t.split("/").map(safeDecode).join("/");
}
