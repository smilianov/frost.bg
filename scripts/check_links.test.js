...
//
// СЪРЦЕТО НА ФАЙЛА СА ТАБЛИЦИТЕ. Този инструмент мина през осем кръга поправки
// и почти всеки от тях счупи съседа си незабелязано — защото репродукциите
// живееха в докладите на ревюера, а не в пакета. Затова тук влиза репродукция
// с коментар откъде идва, и ред се маха само съзнателно: ако нечия „поправка“
// го обърне, това се вижда по име и по произход, а не се случва в тишина.
//
// Редовете идват от прегледите в `.superpowers/sdd/2026-09-25-frost-bg-guide/`.
// НЕ твърдя, че вътре е всичко — два пъти вече такова твърдение се оказа
// по-силно от направеното. Смисълът на файла е друг: тук е МЯСТОТО, където
// влиза следващата находка. Намери ли се нова, редът ѝ идва при тези.
//
...
const HISTORY_CASES = [
  ...
  {
    from: "повторен преглед 5 — сложният API адрес с окончания %zz и %FF",
    name: "/api/../a%2Fb/../api%2Fv1/missing{%zz,%FF}.html",
    files: md([
      "[x](/api/../a%2Fb/../api%2Fv1/missing%zz.html)",
      "[y](/api/../a%2Fb/../api%2Fv1/missing%FF.html)",
    ]),
    expect: {
      checked: 3,
      dead: [
        "3 /api/../a%2Fb/../api%2Fv1/missing%zz.html",
        "4 /api/../a%2Fb/../api%2Fv1/missing%FF.html",
      ],
    },
  },
  {
    from: "task-3-rereview2 — многоредов inline span",
    name: "inline span през два реда не изяжда съседната връзка",
    files: md(["`код", "през два реда` и после [x](/missing.html)"]),
    expect: { checked: 2, dead: ["4 /missing.html"] },
  },
  {
    from: "повторен преглед 5 — общата грешка на маските във вариант CRLF",
    name: "общата грешка на двете маски държи и с CRLF окончания",
    files: {
      "README.md": [
        "~~~html", "<!--", "~~~", "~~~html", "-->", "~~~", "<!--", "```", "-->",
        "[broken](/missing.html)", "<!--", "~~~", "```", "-->", "", "[база](docs/base.md)",
      ].join("\r\n"),
      "docs/base.md": "x",
    },
    expect: { checked: 1, unparsable: ["4 смесени огради"] },
  ...
  {
    from: "повторен преглед 5 — контрастът към тихия случай в Markdown",
    name: "привидни огради в ИСТИНСКИ .html не скриват връзката",
    files: html(["<div>", "~~~", '<a href="/missing.html">broken</a>', "~~~", "</div>"]),
    file: SITE,
    expect: { checked: 2, dead: ["4 /missing.html"] },
  },
];
...
const LIMIT_CASES = [
  ...
  {
    from: "task-3-rereview — reference дефиниции (приета граница)",
    name: "ОГРАНИЧЕНИЕ: дублирана и неизползвана reference дефиниция не се разпознават",
    files: md(["[ref]: docs/missing.md", "[ref]: docs/other-missing.md", "[unused]: docs/third-missing.md"]),
    expect: { checked: 1 },
  },
  {
    from: "task-3-rereview — reference адрес на следващ ред (приета граница)",
    name: "ОГРАНИЧЕНИЕ: reference дефиниция с адрес на следващия ред не се разпознава",
    files: md(["[ref]:", "  docs/missing.md"]),
    expect: { checked: 1 },
  },
  {
    from: "повторен преглед 5 — условието към третия случай",
    name: "ОГРАНИЧЕНИЕ, ТИХО: /api/. и /api/x/.. при СЪЩЕСТВУВАЩ site/api минават за живи",
    files: md(["[x](/api/.)", "[y](/api/x/..)"], { "site/api/index.html": "<p>x</p>" }),
    expect: { checked: 3 },
  },
  {
    from: "повторен преглед 5 — кодиран разделител извън site/ (заварено)",
    name: "ОГРАНИЧЕНИЕ, ТИХО: /..%2Fpackage.json е жив през файл ИЗВЪН site/",
    files: md("[x](/..%2Fpackage.json)", { "package.json": "{}" }),
    expect: { checked: 2 },
  },
  {
    from: "повторен преглед 5 — контролата към горния ред",
    name: "самостоятелното /../package.json правилно излиза над корена",
    files: md("[x](/../package.json)", { "package.json": "{}" }),
    expect: { checked: 1, unparsable: ["3 над корена на сайта"] },
  },
  {
...
test("съществуващо, но празно дърво проваля проверката", () => {
  const root = fixture({ ".gitkeep": "" });
  try {
