...
// C1: „една седмица по-късна сеитба“ звучеше като достатъчно отместване.
// Окончателният текст е решение на СОБСТВЕНИКА: диапазон, без твърдо число, за
// да не подведе никого. Мярката, с която е взето: за 1 902-те клетки с
// пролетни дати разликата сигурна−типична има медиана 13 дни, но стига до 30 —
// 1 282 клетки (67%) попадат в 7–14, а 571 (30%) са НАД 14. Затова
//   - „обикновено“ не е стил: без него изречението твърди нещо за всички места
//     и е невярно за онези 571 клетки;
//   - „числото за твоята точка е на екрана“ е предпазителят срещу
//     подценяването: който е в Родопите, вижда своето число.
// Двете числа на диапазона стоят в ЕДНО очакване, с граници: 7→5, 14→21 или
// „7 и 140“ поваля реда (мутациите са пускани).
test("съветът за сеитба дава диапазон с уговорка, не твърдо число", () => {
  const bgP = paragraphContaining(bg, "Съвет");
  const enP = paragraphContaining(en, "Tip");
  assert.ok(includesExactPhraseWithTrailingUnit(bgP, "между 7 и 14 дни"), "bg: диапазонът 7–14 дни");
