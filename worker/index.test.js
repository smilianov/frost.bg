...
const REV = (map = "osm", geocoder = "openmeteo") => encodeURIComponent(`0.2.1|${grid.computed}|${map}|${geocoder}`);
...
test("/api/v1/frost: нова мрежа (друг grid.computed) при запазен кеш -> miss, не старият отговор", async () => {
  ...
  try {
    ...
    assert.ok(store.has(`https://frost.bg/api/v1/frost?rev=${encodeURIComponent("0.2.1|2031-01-05|osm|openmeteo")}&lat=42.184&lon=24.929`),
      [...store.keys()].join(" "));
  } finally {
