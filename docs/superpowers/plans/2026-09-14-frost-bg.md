...
**Измерено с истинска проба (14 септември 2026, `~/.cdsapirc` е на място, лицензите
са приети, venv-ът `grid/.venv-cds` вече съществува с `cdsapi 0.7.7` и
`netCDF4 1.7.4` — не го създавай наново, само провери):**

- годишният файл идва като **чист `.nc`** (не zip) — `_extract_nc` трябва да
  приема и двете; 1,3 MB за 2025; опашката на CDS — **~25 минути** за
  година; геопотенциалът — 26 KB за 30 s;
- `t2m(valid_time, latitude, longitude)` в Kelvin, `float32`; **`valid_time`
  е `days since 2025-01-01 00:00:00`** (не секунди!) — затова
  `netCDF4.num2date` с `units` е задължителен, не ръчна аритметика; има и
  скаларна променлива `number`;
- `latitude` е **низходяща** 44.3 → 41.2 (32), `longitude` 22.3 → 28.7 (65) —
  точно нашата решетка, стойностите са закръглени до 0,1 при float32
  (закръгляй с `round(float(x), 1)`);
- геопотенциалът има променливи `z, longitude, latitude, time` (измерението е
  `time`, не `valid_time`); `z` за 42.2/24.9 → **99 м** (Open-Meteo даваше
  157 — различен релефен модел; ползваме моделния, той е сметнал сланата);
- тестовият NetCDF в `tests_cds.py` да отразява това: `valid_time` с
  `units = "days since <YYYY>-01-01 00:00:00"` и стойности 0..364/365,
  `number` скалар; за геопотенциала — измерение `time`.

**Паралелност:** `fetch_cds.py --parallel 3` — три години наведнъж
(`concurrent.futures.ThreadPoolExecutor`, всеки поток със свой
`cdsapi.Client()`); по подразбиране 3, `--parallel 1` за последователно.
Продължаването при прекъсване остава: готовите файлове се прескачат,
`.part` файлове се презаписват.
