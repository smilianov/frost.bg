...
Expected: `19 успешни, 0 неуспешни` (16 от първата секция + 3 от втората). Това е копие — минава веднага; доказателството е бройката и че `fe` е локалният модул (`python3 -c "import frost_estimate, inspect; print(inspect.getfile(frost_estimate))"` от `grid/` печата пътя в `grid/`).
...
```bash
git add grid/compute_grid.py grid/tests_grid.py grid/grid.json
...
```bash
git add worker/frost.js worker/texts.js worker/frost.test.js
