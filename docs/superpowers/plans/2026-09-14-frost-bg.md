...
<footer class="foot">
  <span id="source"></span> · <a href="https://open-meteo.com/" rel="noopener">Open-Meteo</a> · <a href="/api/v1/config">API</a>
</footer>
...
(async () => {
  ...
  if (cfg.grid?.period) $("source").textContent = `${t.source}: ERA5 ${cfg.grid.period.start}–${cfg.grid.period.end}`;
...
- [ ] **Step 4: Истинската мрежа — пусни на фон**

От `grid/`: `nohup python3 compute_grid.py --pause 1.5 > compute.log 2>&1 &`. Проверявай `tail -3 grid/compute.log`. При лимит на Open-Meteo (429 три пъти на точка) скриптът връща 1 със списък на пропуснатите — пусни го пак по-късно; готовите се прескачат. Когато обобщението каже `готови: 2080 / 2080` и `grid.json … synthetic=False`:

```bash
cd grid && python3 tests_grid.py && cd .. && node --test worker/
git add grid/grid.json
git commit -m "Мрежата: истинските 2 080 точки от ERA5 1996–2025"
```

`grid/compute.log` и `grid/cells.jsonl` не влизат в репото (`.gitignore` — добави `grid/compute.log`).
