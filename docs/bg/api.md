...
Cloudflare, изрично, не само по хедъра; ключът носи и ревизия (версия,
дата на мрежата, карта, геокодер). Защо и какво значи това при deploy —
„Кешът на API-то“ в [`operations.md`](operations.md).
...
- доставчик по `env.GEOCODER`: `openmeteo` (по подразбиране, безплатен,
  ...
  доставчика; записи без име или с координати извън ±90°/±180° се
  пропускат; таймаут 8 секунди;
...
Кеш: `Cache-Control: public, max-age=86400`; ключът носи ревизия, така че
смяна на картата/геокодера или нова мрежа се вижда веднага, не след
денонощие (виж [`operations.md`](operations.md)).
