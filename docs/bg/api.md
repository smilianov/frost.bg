...
`years_used` е броят приети години — тези с поне 300 валидни дни — и е равен
на броя редове в `years`. Всеки ред е `[година, последна_пролетна_слана,
първа_есенна_слана]` с дати `MM-DD`; сезонно `null` означава, че в наличните
данни за тази половина на годината не е записана слана. Такава година влиза в
...
`elevation_m` е закръглен до цял метър: `null` означава липсваща стойност,
...
- `elevation` — дали `/api/v1/elevation` е включен (превключвателят
  ...
  Точно като картата/геокодера, тази стойност **влиза** в ревизията на
  кеша на `/config` по-долу (собствен `configRev()`, отделен от
  `/frost`/`/geocode`) — смяна на превключвателя стига до ръба веднага при
  нов deploy. Браузърът получава промяната при следваща заявка след
  изтичане на кешираното копие (5 минути); отворената страница не
...
| `not_found` | 404 | `GET` към непознат `/api/*` път, или към `/elevation` при `ELEVATION = "off"` |
| `not_found` | 405 | метод, различен от `GET`/`OPTIONS`, към който и да е `/api/*` път (проверката на метода предхожда маршрутизирането) — отговорът носи и `Allow: GET, OPTIONS` |
