...
also works as a free **API** for other applications (including
...
- [`operations.md`](operations.md) — how the grid is computed and
  refreshed, how to run and test the project locally, what is still missing
...
see [`operations.md`](operations.md) for how and when.
...
for it (rare; only a handful of cells).
...
Bulgaria only, and only the **precomputed** grid — a request outside the
country's bounding box gets a polite refusal, not an approximation from a
neighboring cell. Details on the boundaries and error codes are in
[`api.md`](api.md).
